...
@section('content')
    <div class="container mx-auto px-4 py-16 ">
        <div class=" flex flex-col md:flex-row pb-12 lg:px-20 border-b border-gray-800">
            <div class="w-full items-center flex flex-col md:w-[40%] lg:w-[30%]">
                <img src="{{ $actor['profile_path'] }}" alt="profile image" class="w-[80%] lg:w-[100%]">
                <ul class="flex items-center mt-4 justify-around">
                    @if ($social['facebook'])
                        <li>
            ...
            <div class="md:ml-8 lg:ml-16 md:w-[60%]">
                <h2 class="text-4xl mt-4 md:mt-0 font-semibold">{{ $actor['name'] }}</h2>
                <div class="mt-3 flex flex-wrap items-center text-gray-400 text-sm">
        ...
        <div class="pb-12 border-b border-gray-800">
                ...
                @foreach ($knownForMovies as $movie)
                    <div class="mt-8">
                        <a href="{{ $movie['linkToPage'] }}"><img src="{{ $movie['poster_path'] }}" alt="poster"
                                class="w-full hover:opacity-75 transition ease-in-out duration-150"></a>
                    ...
                    @foreach ($credits as $credit)
                        <li>
                            ...
                            @if ($credit['media_type'] == 'movie')
                                Filme: <strong><a href="{{ $credit['linkToPage'] }}"
                                        class="hover:underline">{{ $credit['title'] }}</a></strong>
                                como {{ $credit['character'] }}
                            @else
                                Seriado: <strong><a href="{{ $credit['linkToPage'] }}"
                                        class="hover:underline">{{ $credit['title'] }}</a></strong>
                                como {{ $credit['character'] }}
                            @endif
                        </li>
